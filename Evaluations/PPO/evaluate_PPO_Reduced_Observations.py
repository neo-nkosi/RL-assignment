    ...
    def setup_observations(self):
        ...
        self._gym_env.observation_space = BoxGymObsSpace(
            self._g2op_env.observation_space,
             attr_to_keep=[       # Lists all the parts of the observation space that we have decided to keep
            "rho",                  
            "line_status",        
            "topo_vect",             
...
def evaluate_agent(env, model, num_episodes=100, max_steps=10000, save_data=False, data_filename=None):
    ...
    print(f"Average Total Reward over {num_episodes} episodes: {avg_reward} ± {std_reward}")
    print(f"Average Episode Length over {num_episodes} episodes: {avg_length} ± {std_length}")
...
def main():
    ...
    model = PPO.load("ppo_grid2op_agent_reduced_obs", env=env)

    # Evaluate the trained agent
    print("Evaluating the reduced_obs trained agent...")
    ...
    print(f"Trained Agent (reduced_obs) Average Reward: {avg_reward_trained} ± {std_reward_trained}")
    print(f"Trained Agent (reduced_obs) Average Episode Length: {avg_length_trained} ± {std_length_trained}")
